...
provide(MizuAlertDialogDismissibleKey, props.dismissible);

const contentClasses = computed(() => {
  const classes = ["mizu-alert-dialog__content", `mizu-alert-dialog__content--${props.size}`];
  if (props.fullscreen) classes.push("mizu-alert-dialog__content--fullscreen");
  if (props.scrollable) classes.push("mizu-alert-dialog__content--scrollable");
  if (!props.transition) classes.push("mizu-alert-dialog__content--no-transition");
...
.mizu-alert-dialog__content:focus {
  outline: none;
}

.mizu-alert-dialog__content[data-state="open"] {
  animation: alertDialogContentIn 200ms ease-out;
}

.mizu-alert-dialog__content[data-state="closed"] {
...
/* Scrollable mode */
.mizu-alert-dialog__content--scrollable {
  max-height: 90vh;
  overflow-y: auto;
}

/* No transition mode */
.mizu-alert-dialog__content--no-transition[data-state="open"],
.mizu-alert-dialog__content--no-transition[data-state="closed"] {
  animation: none;
}
</style>
