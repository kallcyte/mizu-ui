...
const styleVars = computed(() => ({
  "--mizu-alert-bg": variantColor.value.bg,
  "--mizu-alert-text": variantColor.value.text,
  "--mizu-alert-border": variantColor.value.border,
  "--mizu-alert-icon": variantColor.value.icon,
}));
...
<template>
  ...
  >
    ...
    <div class="mizu-alert__content">
      <div v-if="title" class="mizu-alert__title">{{ title }}</div>
      <div class="mizu-alert__body">
        <slot />
      </div>
    </div>
    <button
...
.mizu-alert {
  ...
  background-color: var(--mizu-alert-bg);
  border-color: color-mix(in srgb, var(--mizu-alert-border) 25%, transparent);
  color: var(--mizu-alert-text);
  padding: 10px 12px;
  font-size: 12px;
  line-height: 1.45;
  opacity: 1;
