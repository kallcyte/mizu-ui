...
const showContent = computed(() => {
  ...
  if (slots.default)
    return (slots.default() as any[]).some(
      (v: any) => typeof v?.children === "string" && v.children.length > 0
    );
  return false;
});
const cvStyle = computed((): Record<string, string> => {
  ...
  const w = r("width");
  if (w != null) s.width = px(w)!;
  const h = r("height");
  if (h != null) s.height = px(h)!;
  const rd = r("radius");
  if (rd != null) s.borderRadius = px(rd)!;
  const bc = cssColor(r("baseColor"));
  if (bc) s["--mizu-skeleton-base"] = bc;
  const hc = cssColor(r("highlightColor"));
  if (hc) s["--mizu-skeleton-highlight"] = hc;
  const dur = r("animationDuration");
  if (dur != null) s["--mizu-animation-duration"] = dur + "s";
  const dir = r("animationDirection");
  if (dir) s["--mizu-animation-direction"] = dir;
  const ea = r("enableAnimation");
  if (ea === false) s["--mizu-animation-display"] = "none";
  const cbg = r("customHighlightBackground");
  if (cbg) s["--mizu-skeleton-custom-gradient"] = cbg;
...
function stkSty(i: number, t: number): Record<string, string> {
  return {
    ...cvStyle.value,
    width: i < t - 1 ? (r("width") != null ? px(r("width")!)! : "100%") : "60%",
  };
}
</script>
...
.mizu-skeleton {
  display: block;
  background-color: var(--mizu-skeleton-base, #e5e7eb);
  border-radius: var(--radius-default, 6px);
}
[data-theme="dark"] .mizu-skeleton {
  background-color: var(--mizu-skeleton-base, #1f2937);
}
.mizu-skeleton--text {
  height: 0.85em;
  max-width: 100%;
...
.mizu-skeleton--shimmer {
  position: relative;
  overflow: hidden;
}
.mizu-skeleton--shimmer::after {
  content: "";
  display: var(--mizu-animation-display, block);
  position: absolute;
  inset: 0;
  height: 100%;
  background-image: var(
    --mizu-skeleton-custom-gradient,
    linear-gradient(
      90deg,
...
@keyframes mizu-skeleton-shimmer {
  100% {
    transform: translateX(100%);
  }
}
.mizu-skeleton--pulse {
  animation: mizu-skeleton-pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite;
}
@keyframes mizu-skeleton-pulse {
  0%,
  100% {
    opacity: 1;
  }
...
.mizu-skeleton-stack > .mizu-skeleton--text {
  @apply max-w-full;
}
