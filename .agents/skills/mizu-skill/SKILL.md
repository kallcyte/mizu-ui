...
14. **Never commit, push, tag, publish, or create/update/delete a release without explicit permission in the user's latest message.** Permission is one-shot and applies only to the specific operation(s) named in that message. Never infer permission from previous messages, prior releases, phrases such as "mark as ready", "recheck docs", or "update changelog", or from completing release-preparation work. After any subsequent change or review request, stop after validation and ask for fresh permission before performing repository or release operations. If the user requests only some operations (for example, commit but not push), perform only those operations. Before executing, state exactly which commit/push/tag/publish/release operations you are about to perform.
